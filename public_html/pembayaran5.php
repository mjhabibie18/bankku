      ...
      <table class="table-responsive table-condensed">
        <tr>
          <form id="myForm" method="get" action="<?php $_SERVER['PHP_SELF'] ?>">
            ...
            <?php
              if($_SESSION['rek'] == ""){ ?>
                <td><input class="form-control" id="id_t" name="id_t" type="text"></td>
              <?php } else { ?>
                <td><input class="form-control" id="id_t" name="id_t" type="text"></td>
             <?php }
            ?>
        ...
        <form id="form1" action="https://alphawaresport.000webhostapp.com/service_updatePembelianWildan.php" method="post">
        <tr>
          <td><label for="ex3">Nama Tujuan</label></td>
          <td><input type='text' name='nama_penerima' class="form-control" id='nama_penerima' value='<?php echo $_SESSION["wildan.com"]; ?>' readonly /></td>
        </tr>
        <?php
        ...
        //   echo "<td><input type='text' class='form-control' name='nama_penerima' value='' id='nama_penerima' readonly /></td>";
          ...
          $transaction_id = $_GET['id_t'];
          $_SESSION['rek'] = $transaction_id;
          $curl = curl_init();
          curl_setopt($curl, CURLOPT_URL, "https://alphawaresport.000webhostapp.com/service_ambilDataWildan.php?id_t=$transaction_id");
          ...
          foreach ($xml as $key){
            if($key == "no rekening tidak ada"){
              ...
              echo "</tr>";
              echo "<script>document.getElementById('id_t').value = '$_GET[id_t]';</script>";
              echo "<input type='hidden' class='form-control' name='id_tr' id='id_tr' value='$_GET[id_t]' readonly />";
            }
          }
        }
        ?>
