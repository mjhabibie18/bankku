      ...
      <table class="table-responsive table-condensed">
        <tr>
          <form id="myForm" method="get" action="<?php $_SERVER['PHP_SELF'] ?>">
            ...
            <?php
              if($_SESSION['rek'] == ""){ ?>
                <td><input class="form-control" id="tlp" name="tlp" type="telp"></td>
              <?php } else { ?>
                <td><input class="form-control" id="tlp" name="tlp" type="text"></td>
             <?php }
            ?>
          </form>
        </tr>
        <form id="form1" action="sister_server/service_pembayaran.php" method="post">
        <tr>
          <td><label for="ex3">Nama Tujuan</label></td>
          <td><input type='text' name='nama_penerima' class="form-control" id='nama_penerima' value='<?php echo $_SESSION["kongkow"]; ?>' readonly /></td>
        </tr>
        <?php
        ...
        //   echo "<td><input type='text' class='form-control' name='nama_penerima' value='' id='nama_penerima' readonly /></td>";
          ...
          $_SESSION['rek'] = $no_tlp;
          $curl = curl_init();
          curl_setopt($curl, CURLOPT_URL, "http://kongkow3logi.com/service_ambilDataDanar.php?tlp=$no_tlp");
          ...
          foreach ($xml as $key){
            if($key == "no rekening tidak ada"){
              ...
              echo "</tr>";
              echo "<script>document.getElementById('tlp').value = '$_GET[tlp]';</script>";
              echo "<input type='hidden' class='form-control' name='nomertlp' id='nomertlp' value='$_GET[tlp]' readonly />";
            }
          }
        }
        ?>
